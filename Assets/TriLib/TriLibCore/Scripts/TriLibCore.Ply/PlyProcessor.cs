...
namespace TriLibCore.Ply
{
    public partial class PlyProcessor
    {
        ...
        {
            _reader = plyReader;
            var elements = new OrderedDictionary<long, PlyElement>();
                    ...
                    case _property_token:
                        {
                            ...
                            if (propertyType == PlyPropertyType.List)
                            {
                                ...
                                var property = new PlyListProperty { Type = propertyType, CounterType = counterPropertyType, ItemType = itemPropertyType };
                                activeElement.AddProperty(propertyName, property);
                            }
                            else
                            {
                                var propertyName = plyStreamReader.ReadToken(true);
                                var property = new PlyProperty { Type = propertyType };
                                activeElement.AddProperty(propertyName, property);
                            }
                            break;
                        }
            ...
            if (elements.TryGetValue(_vertex_token, out var vertexElementForSizing))
            {
                _allVertices = new List<Vector3>(vertexElementForSizing.Count);
                _allNormals = new List<Vector3>(vertexElementForSizing.Count);
                _allColors = new List<Color>(vertexElementForSizing.Count);
                _allUVs = new List<Vector2>(vertexElementForSizing.Count);
            } else
            {
            ...
            foreach (var element in elements.Values)
            {
                ...
                for (var i = 0; i < element.Count; i++)
                {
                    ...
                    foreach (var property in element.Properties.Values)
                    {
                        ...
                        element.SetData(i, propertyOffset, value);
                        propertyOffset++;
                    }
                }
            }
            ...
            if (!plyReader.AssetLoaderContext.Options.LoadPointClouds && plyReader.AssetLoaderContext.Options.ImportMaterials)
            {
                if (elements.TryGetValue(_material_token, out var materialElement))
                {
                    ...
                    for (var i = 0; i < materialElement.Count; i++)
                    {
                        ...
                        var diffuse_red = materialElement.GetPropertyFloatValue(_diffuse_red_token, i) / 255f;
                        var diffuse_green = materialElement.GetPropertyFloatValue(_diffuse_green_token, i) / 255f;
                        var diffuse_blue = materialElement.GetPropertyFloatValue(_diffuse_blue_token, i) / 255f;
                        material.AddProperty("diffuse", new Color(diffuse_red, diffuse_green, diffuse_blue), false);
                        var specular_red = materialElement.GetPropertyFloatValue(_specular_red_token, i) / 255.0f;
                        var specular_green = materialElement.GetPropertyFloatValue(_specular_green_token, i) / 255.0f;
                        var specular_blue = materialElement.GetPropertyFloatValue(_specular_blue_token, i) / 255.0f;
            ...
            if (plyReader.AssetLoaderContext.Options.ImportMeshes && elements.TryGetValue(_vertex_token, out var vertexElement))
            {
                ...
                var gProp = vertexElement.GetProperty(_green_token) ?? vertexElement.GetProperty(_diffuse_green_token);
                var bProp = vertexElement.GetProperty(_blue_token) ?? vertexElement.GetProperty(_diffuse_blue_token);
            ...
            _geometryGroup.Setup(_reader.AssetLoaderContext, 3, 1);
            ...
            if (plyReader.AssetLoaderContext.Options.LoadPointClouds)
            {
                ...
                for (var i = 0; i < _allVertices.Count; i++)
                {
                    geometry.AddVertex(plyReader.AssetLoaderContext,
                        i,
                        position: _allVertices[i],
                        normal: ListUtils.FixIndex(i, _allNormals),
                        tangent: default,
                        color: ListUtils.FixIndex(i, _allColors),
                        uv0: ListUtils.FixIndex(i, _allUVs));
                }
            }
            else if (plyReader.AssetLoaderContext.Options.ImportMeshes && elements.TryGetValue(_face_token, out var faceElement))
            {
                ...
                for (var j = 0; j < faceElement.Count; j++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Debug.LogWarning($"[PlyProcessor] Face {j} has {vertexIndices.Count} vertices (max supported is 255) and will be skipped.");
                        }
                    }
                    lists[vertexIndicesListIndex] = null;
                    if (texCoord != null)
                    {
                        lists[faceElement.GetListIndex(texCoordProp, j)] = null;
                    }
                }
                plyReader.UpdateLoadingPercentage(1f, (int)PlyReader.ProcessingSteps.PostProcessGeometries);
                faceElement.Data = null;
            }
        ...
        private IVertexData BuildVertexData(PlyGeometry triGeometry, IGeometryGroup geometryGroup, int vertexIndex, List<PlyValue> texCoord, PlyListProperty texCoordProp, int texCoordIndex = -1)
        {
            ...
            if (texCoord != null)
            {
                var u = PlyValue.GetFloatValue(texCoord[texCoordIndex * 2 + 0], texCoordProp.ItemType);
                var v = PlyValue.GetFloatValue(texCoord[texCoordIndex * 2 + 1], texCoordProp.ItemType);
                uv = new Vector2(u, v);
            }
            else
            {
        ...
        private void AddVertex(int vertexIndex, PlyGeometry geometry, List<PlyValue> texCoord, int texCoordIndex = -1, PlyListProperty texCoordProp = null)
        {
            Vector2 uv;
            if (texCoord != null)
            {
            ...
            geometry.AddVertex(_reader.AssetLoaderContext,
                ...
                tangent: default,
                color: ListUtils.FixIndex(vertexIndex, _allColors),
                uv0: uv);
        }
