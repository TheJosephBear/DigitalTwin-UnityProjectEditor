        ...
        private void PostProcessDeformers()
        {
            for (var subDeformerIndex = 0; subDeformerIndex < Document.SubDeformers.Count; subDeformerIndex++)
            {
                ...
                if (deformer.Geometry != null)
                {
                    ...
                    var meshWorldMatrix = model.Matrices.ComputeMatrix(Reader.AssetLoaderContext); // no parent walk
                    var newTransform = subDeformer.TransformLink.inverse * meshWorldMatrix;
