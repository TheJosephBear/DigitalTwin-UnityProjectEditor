...
namespace TriLibCore.Stl
{
    public class StlProcessor
    {
        ...
        private static readonly byte[] _colorHeaderBytes = Encoding.ASCII.GetBytes(ColorHeader);
        ...
        private static bool CheckForColorHeader(Stream stream)
        {
            ...
            while (true)
            {
                ...
                if (nextByte == _colorHeaderBytes[index])
                {
                    index++;
                    if (index == _colorHeaderBytes.Length)
                    {
                        return true;
                    }
                }
