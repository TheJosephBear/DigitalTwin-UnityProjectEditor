...
namespace TriLibCore.Dae
{
    public class DaeProcessor
    {
        ...
        private static int[] SplitToInt(string data, bool reverse, int biggestOffset)
        {
            ...
            while (i < length)
            {
                while (i < length && IsSplitChar(data[i]))
                {
                    i++;
                }
                if (i >= length)
            ...
            while (i < length)
            {
                ...
                intData[index++] = negative ? -value : value;
            }
            if (reverse && biggestOffset > 0)
            {
                var groups = intData.Length / biggestOffset;
                for (var g = 0; g < groups / 2; g++)
                {
                    var a = g * biggestOffset;
                    var b = (groups - 1 - g) * biggestOffset;
                    for (var j = 0; j < biggestOffset; j++)
                    {
                        var tmp = intData[a + j];
                        intData[a + j] = intData[b + j];
                        intData[b + j] = tmp;
                    }
                }
            }
            ...
            if (node.instance_node != null)
            {
                ...
                if (libraryNodes != null)
                {
                    ...
                    if (itemWithId != null)
                    {
                        ...
                        if (itemWithId.node1 != null)
                        {
                            if (node.node1 == null)
                            {
                                node.node1 = itemWithId.node1;
                            }
                            else
                            {
                                var mergedNodes = new node[node.node1.Length + itemWithId.node1.Length];
                                node.node1.CopyTo(mergedNodes, 0);
                                itemWithId.node1.CopyTo(mergedNodes, node.node1.Length);
                                node.node1 = mergedNodes;
                            }
                        }
                    }
                }
        ...
        private IRootModel ProcessScene()
        {
            ...
            _scaleFactor = _collada.asset?.unit != null ? (float)_collada.asset.unit.meter : 1f;
            ...
            if (libraryVisualScenes != null)
            {
                ...
                var extraInstanceGeometryModels = new List<DaeModel>();
                foreach (var existingModel in _models)
                {
                    var existingNode = existingModel.Node;
                    if (existingNode?.instance_controller == null && existingNode?.instance_geometry != null && existingNode.instance_geometry.Length > 1 && existingModel.Parent != null)
                    {
                        for (var instanceIndex = 1; instanceIndex < existingNode.instance_geometry.Length; instanceIndex++)
                        {
                            var extraNode = new node
                            {
                                instance_geometry = new[] { existingNode.instance_geometry[instanceIndex] }
                            };
                            var extraModel = new DaeModel
                            {
                                Name = existingModel.Name,
                                Visibility = existingModel.Visibility,
                                Parent = existingModel.Parent,
                                LocalPosition = existingModel.LocalPosition,
                                LocalRotation = existingModel.LocalRotation,
                                LocalScale = existingModel.LocalScale,
                                Node = extraNode
                            };
                            existingModel.Parent.Children ??= new List<IModel>();
                            existingModel.Parent.Children.Add(extraModel);
                            _rootModel.AllModels.Add(extraModel);
                            extraInstanceGeometryModels.Add(extraModel);
                        }
                    }
                }
                _models.AddRange(extraInstanceGeometryModels);
