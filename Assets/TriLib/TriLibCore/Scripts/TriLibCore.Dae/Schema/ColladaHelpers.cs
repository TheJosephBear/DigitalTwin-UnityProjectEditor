    ...
    public static class COLLADAHelper
    {
        ...
        public static T GetItemWithId<T>(this IList<T> items, string id) where T : class
        {
            if (items != null && id != null)
            {
                ...
                var rawId = hashIndex >= 0 ? id.Substring(hashIndex + 1) : id;
                foreach (var item in items)
                {
                    switch (item)
                    {
                        case source source when source.id == rawId:
                            return item;
                        case geometry geometry when geometry.id == rawId:
                            return item;
                        case visual_scene visualScene when visualScene.id == rawId:
                            return item;
                        case effect effect when effect.id == rawId:
                            return item;
                        case controller controller when controller.id == rawId:
                            return item;
                        case sampler sampler when sampler.id == rawId:
                            return item;
                        case node node when node.id == rawId:
                            return item;
                    }
                }
