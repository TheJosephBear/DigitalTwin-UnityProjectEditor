...
namespace TriLibCore.Utils
{
    ...
    public class JsonParser
    {
        ...
        private byte[] _stringValueBuffer;
                    ...
                    case QuoteChar:
                        {
                            ...
                            for (; ; )
                            {
                                ...
                                if (peek == QuoteChar && !escaped)
                                {
                                    break;
                                }
                                escaped = peek == BackslashChar && !escaped;
        ...
        public struct JsonValue : IEnumerable<JsonValue>
        {
            ...
            public static unsafe string GetStringValue(JsonParser jsonParser, long position, int length)
            {
                ...
                if (length > 2048)
                {
                    if (jsonParser._stringValueBuffer == null || jsonParser._stringValueBuffer.Length < length)
                    {
                        jsonParser._stringValueBuffer = new byte[length];
                    }
                    var bytes = jsonParser._stringValueBuffer;
