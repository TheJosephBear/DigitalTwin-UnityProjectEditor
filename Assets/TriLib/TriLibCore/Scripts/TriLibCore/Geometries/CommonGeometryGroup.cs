...
namespace TriLibCore.Geometries
{
    ...
    public class CommonGeometryGroup : IGeometryGroup
    {
        ...
        private List<int> _rawBoneWeightVertices;
        private List<BoneWeight1> _rawBoneWeightValues;
        private int _maxBoneWeightVertexIndex = -1;
        ...
        /// </summary>
        public int VerticesDataCount => Positions?.Count ?? 0;
        ...
        {
            if (_rawBoneWeightValues == null)
            {
                _rawBoneWeightVertices = new List<int>(VerticesCapacity);
                _rawBoneWeightValues = new List<BoneWeight1>(VerticesCapacity);
            }
            _rawBoneWeightVertices.Add(vertexIndex);
            _rawBoneWeightValues.Add(boneWeight1);
            if (vertexIndex > _maxBoneWeightVertexIndex)
            {
                _maxBoneWeightVertexIndex = vertexIndex;
            }
        }
        ...
        public IEnumerable GenerateMesh(AssetLoaderContext assetLoaderContext, GameObject meshGameObject, IModel meshModel)
        {
            ...
            if (VerticesDataCount > 0)
            {
                ...
                var releaseStagingData = !assetLoaderContext.Options.LoadPointClouds && !assetLoaderContext.Options.ReadEnabled && assetLoaderContext.Options.BlendShapeMapper == null;
                ...
                if (!assetLoaderContext.Options.LoadPointClouds)
                {
                    if (_rawBoneWeightValues != null && _rawBoneWeightValues.Count > 0)
                    {
                        var boneWeightVertexCount = _maxBoneWeightVertexIndex + 1;
                        var boneWeightEntryCount = _rawBoneWeightValues.Count;
                        var boneWeightOffsets = new int[boneWeightVertexCount + 1];
                        for (var i = 0; i < boneWeightEntryCount; i++)
                        {
                            boneWeightOffsets[_rawBoneWeightVertices[i] + 1]++;
                        }
                        for (var i = 0; i < boneWeightVertexCount; i++)
                        {
                            boneWeightOffsets[i + 1] += boneWeightOffsets[i];
                        }
                        var groupedBoneWeights = new BoneWeight1[boneWeightEntryCount];
                        var boneWeightCursor = new int[boneWeightVertexCount];
                        for (var i = 0; i < boneWeightEntryCount; i++)
                        {
                            var vertexIndex = _rawBoneWeightVertices[i];
                            groupedBoneWeights[boneWeightOffsets[vertexIndex] + boneWeightCursor[vertexIndex]++] = _rawBoneWeightValues[i];
                        }
                        _rawBoneWeightVertices = null;
                        _rawBoneWeightValues = null;
                        if (assetLoaderContext.Options.LimitBoneWeights)
                        {
                            ...
                            for (var i = 0; i < boneWeights.Length; i++)
                            {
                                ...
                                if (vertexIndex < 0 || vertexIndex >= boneWeightVertexCount)
                                {
                                    continue;
                                }
                                var start = boneWeightOffsets[vertexIndex];
                                var count = boneWeightOffsets[vertexIndex + 1] - start;
                                if (count == 0)
                                {
                                    continue;
                                }
                                boneWeights[i] = GeometryGroupUtils.LimitBoneWeights(groupedBoneWeights, start, count);
                            }
                        ...
                        else
                        {
                            ...
                            var allWeightsList = new List<BoneWeight1>(boneWeightEntryCount);
                            for (var i = 0; i < VerticesDataCount; i++)
                            {
                                ...
                                var start = boneWeightOffsets[vertexIndex];
                                var count = boneWeightOffsets[vertexIndex + 1] - start;
                                bonesPerVertex[i] = GeometryGroupUtils.LimitMaxBoneWeights(allWeightsList, groupedBoneWeights, start, count);
                            }
                ...
                if (releaseStagingData)
                {
                    ...
                    Normals = null;
                    Tangents = null;
                    OriginalVertexIndices = null;
                    _mergedVertices = null;
                    foreach (var geometry in GeometriesData.Values)
                    {
                        if (geometry != null)
                        {
                            geometry.VertexDataIndices = null;
                        }
                    }
                }
            }
