...
namespace TriLibCore.Geometries
{
    ...
    public static class GeometryGroupUtils
    {
        ...
        public static BoneWeight LimitBoneWeights(BoneWeight1[] boneWeights1, int start, int count)
        {
            Array.Sort(boneWeights1, start, count, BoneWeightComparer);
            var boneWeight = new BoneWeight();
            var sum = 0f;
            var bonesCount = Mathf.Min(4, count);
            for (var i = 0; i < bonesCount; i++)
            {
                sum += boneWeights1[start + i].weight;
            }
            if (Mathf.Abs(sum) > 0f)
            {
                var invSum = 1f / sum;
                for (var k = 0; k < bonesCount; k++)
                {
                    var boneWeight1 = boneWeights1[start + k];
                    boneWeight1.weight *= invSum;
                    boneWeights1[start + k] = boneWeight1;
                }
            }
            else
            {
            ...
            if (bonesCount >= 1)
            {
                boneWeight.boneIndex0 = boneWeights1[start].boneIndex;
                boneWeight.weight0 = boneWeights1[start].weight;
            }
            if (bonesCount >= 2)
            {
                boneWeight.boneIndex1 = boneWeights1[start + 1].boneIndex;
                boneWeight.weight1 = boneWeights1[start + 1].weight;
            }
            if (bonesCount >= 3)
            {
                boneWeight.boneIndex2 = boneWeights1[start + 2].boneIndex;
                boneWeight.weight2 = boneWeights1[start + 2].weight;
            }
            if (bonesCount >= 4)
            {
                boneWeight.boneIndex3 = boneWeights1[start + 3].boneIndex;
                boneWeight.weight3 = boneWeights1[start + 3].weight;
            }
            return boneWeight;
        }
        ...
        public static byte LimitMaxBoneWeights(List<BoneWeight1> allWeightsList, BoneWeight1[] weights, int start, int count)
        {
            if (count <= 0)
            {
                return 0;
            }
            Array.Sort(weights, start, count, BoneWeightComparer);
            var bonesCount = (byte)count;
            var sum = 0f;
            for (var j = 0; j < bonesCount; j++)
            {
                sum += weights[start + j].weight;
            }
            if (Mathf.Abs(sum) > 0f)
            {
                var invSum = 1f / sum;
                for (var k = 0; k < bonesCount; k++)
                {
                    var boneWeight1 = weights[start + k];
                    boneWeight1.weight *= invSum;
                    allWeightsList.Add(boneWeight1);
                }
                return bonesCount;
