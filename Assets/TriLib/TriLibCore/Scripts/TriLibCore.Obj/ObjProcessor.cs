...
namespace TriLibCore.Obj
{
    public partial class ObjProcessor
    {
        ...
        private readonly HashSet<string> _processedMaterials = new HashSet<string>();
        ...
        private ContourVertex[] _contourVerticesBuffer = Array.Empty<ContourVertex>();
        ...
        public IRootModel Process(ObjReader reader, Stream stream)
        {
                    ...
                    case f_token when !_reader.AssetLoaderContext.Options.LoadPointClouds && _reader.AssetLoaderContext.Options.ImportMeshes:
                        {
                            ...
                            switch (vertexDataIndices.Count)
                            {
                                ...
                                case 4:
                                    ...
                                    break;
                                default:
                                    if (_contourVerticesBuffer.Length < vertexDataIndices.Count)
                                    {
                                        _contourVerticesBuffer = new ContourVertex[vertexDataIndices.Count];
                                    }
                                    for (var i = 0; i < vertexDataIndices.Count; i++)
                                    {
                                        var vertex = _allVertices[vertexDataIndices[i].VertexIndex];
                                        _contourVerticesBuffer[i] = new ContourVertex(new Vec3(vertex.x, vertex.y, vertex.z), BuildVertexData(_activeGeometry, _activeGeometryGroup, vertexDataIndices, i));
                                    }
                                    Helpers.Tesselate(_contourVerticesBuffer, _reader.AssetLoaderContext, _activeGeometry, _activeGeometryGroup);
                                    break;
                            }
                            break;
                        }
            ...
            _allUVs = null;
            _allColors = null;
            _materialLibraries.Clear();
            _geometryGroups.Clear();
            _models.Clear();
            _textures.Clear();
            _processedMaterials.Clear();
            _contourVerticesBuffer = null;
